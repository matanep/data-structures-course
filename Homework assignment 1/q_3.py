...
if len(keys_1)==0 or len(keys_2)==0 or len(keys_3)==0: #Handling no data.
...
for data,keys in datas_and_keys: #Go over each data set.
    ...
    hash_table_n=1#The hash table we are working on now.
    for Hash_Table in Hash_Tables: #Go over each Hash table
        ...
        print '##Hash Table '+str(hash_table_n)+':',float(count_in)/len(keys)
        hash_table_n+=1
    n+=1
